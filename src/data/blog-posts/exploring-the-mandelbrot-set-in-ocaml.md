...
featured: false
github: "https://github.com/andrewthecodertx/ocaml-mandelbrot"
...
A complex number has two parts: a real part and an imaginary part. We write it as $a + bi$, where $i = \sqrt{-1}$. You can think of complex numbers as points on a 2D plane — the real part is the x-axis, the imaginary part is the y-axis. This is called the **complex plane**.
...
Start with $z_0 = 0$. Pick any point $c$ on the complex plane. Now iterate: square $z$, add $c$, repeat. One of two things happens:

1. The sequence **escapes** — $|z|$ grows without bound, shooting off toward infinity.
2. The sequence **stays bounded** — $|z|$ never exceeds some finite value, no matter how many times you iterate.

The Mandelbrot set is the set of all points $c$ for which the sequence stays bounded. That's the entire definition.

## The Escape Condition

In practice, you can't iterate forever. But there's a useful shortcut: if $|z|$ ever exceeds 2, the sequence is guaranteed to escape. The magnitude $|z| = \sqrt{a^2 + b^2}$, and to avoid the square root we check:
...
For each pixel on screen, we map it to a point $c$ on the complex plane and count how many iterations it takes for $|z|^2$ to exceed 4. If it never does within `max_iteration` steps, we assume the point is in the set.
...
This is what makes it a **fractal**. The boundary's [Hausdorff dimension](https://en.wikipedia.org/wiki/Hausdorff_dimension) is 2, meaning it's so convoluted that it's effectively as complex as a filled area despite being a one-dimensional curve.
...
SDL's coordinate system has $y = 0$ at the top of the window, which is the opposite of the mathematical convention. The code compensates by mapping SDL row 0 to $y_{max}$ and row $h - 1$ to $y_{min}$.
...
Mathematically, zooming just changes the window $[x_{min}, x_{max}] \times [y_{min}, y_{max}]$ on the complex plane. The computation is identical at every scale; only the coordinates change. The set itself has no preferred scale.

At extreme zoom levels, floating-point precision becomes the limiting factor. Standard 64-bit `float` gives roughly 15 significant digits, which allows roughly $2^{52} \approx 10^{15}$ times magnification before the pixels stop resolving new detail. Going deeper requires arbitrary-precision arithmetic or perturbation theory — topics for another day.
...
Click to zoom in. Press `+`/`-` to zoom at the cursor, `hjkl` or `wasd` to pan, `r` to reset, `q` to quit. The source is [on GitHub](https://github.com/andrewthecodertx/ocaml-mandelbrot).
...
- [The Ewing and Schober proof](https://www.jstor.org/stable/2007256) that the Mandelbrot set is connected
- [Mu-Ency](https://mrob.com/pub/muency.html) — an encyclopedia of the Mandelbrot set and its features
